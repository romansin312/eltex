...
void* receive_messages(void* arg) {
    while (running) {
        ...
        sem_wait(&shm->clients[client_index].client_sem);
        
        for (int i = 0; i < shm->clients[client_index].message_count; i++) {
            chat_message *msg = &shm->clients[client_index].messages[i];
            ...
            if (msg->type == MSG_TYPE_TEXT) {
             ...
             for (int j = i; j < shm->clients[client_index].message_count - 1; j++) {
                shm->clients[client_index].messages[j] = shm->clients[client_index].messages[j + 1];
            }
...
void send_message_to_server(chat_message *msg) {
    shm->server_messages[shm->server_messages_count] = *msg;
...
int main(int argc, char *argv[]) {
    ...
    if (shm_fd == -1) {
        printf("shared memory open error\n");
        return 1;
    }
