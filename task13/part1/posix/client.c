...
int main() {
    ...
    mqd_t server_mqd = mq_open(SERVER_MQ_NAME, O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR, &attrs);
    ...
    mqd_t client_mqd = mq_open(CLIENT_MQ_NAME, O_CREAT | O_RDONLY, S_IRUSR | S_IWUSR, &attrs);
    ...
    if (mq_receive(client_mqd, in_buffer, IN_BUFFER_SIZE, NULL) == -1) {
    ...
    if (mq_send(server_mqd, out_buffer, strlen(out_buffer) + 1, 0) == -1) {
